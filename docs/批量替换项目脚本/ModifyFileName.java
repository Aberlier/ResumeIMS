...
public class ModifyFileName {
    static String OLD_PROJECT_NAME = "changqing";// 要被替换的字符串
    static String NEW_PROJECT_NAME = "tanabo";// 新字符串,如果是去掉前缀后缀就留空，否则写上需要替换的字符串

    //这个路径不能含有changqing字符!!!!!!! 先将 changqing-commons-plus 修改为tangyh-commons-plus
    //这个路径不能含有changqing字符!!!!!!! 先将 changqing-commons-plus 修改为tangyh-commons-plus
    //这个路径不能含有changqing字符!!!!!!! 先将 changqing-commons-plus 修改为tangyh-commons-plus
    static String dir = "E:\\work\\idea-workspace\\huasheng2\\Back-end-tanbao";// 文件所在路径，所有文件的根目录，记得修改为你电脑上的文件所在路径
